...
import {getUtilisateur} from "../config/firebase";
...
const UserInfo = () => {
  ...
  const uploadImage = async (file: File | Blob) => {
    try {
      ...
      // Enregistrer l'UID et l'URL de l'image dans Firestore
      const currentUserUid = auth.currentUser?.uid;
      if (currentUserUid) {
        await setDoc(doc(db, "userProfiles", currentUserUid), {
  ...
  const formattedDate = user ? new Date(user.date_creation.seconds * 1000).toLocaleDateString("fr-FR") : null;
      ...
      <div className="max-w-md mx-auto space-y-6">
        ...
        <Card className="bg-crypto-card border-none p-6 relative">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h2 className="text-xl font-bold">{user ? user.pseudo || "Utilisateur 1" : "Utilisateur 1"}</h2>
            </div>
          </div>
        </Card>
        ...
        <Card className="bg-crypto-card border-none p-6 space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between">
              ...
              <span>{user ? (user.email || "Utilisateur@Gmail.com") : "Utilisateur@Gmail.com"}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-crypto-primary">{user ? (user.actif ? "Actif" : "Inactif") : "Inconnu"}</span>
            </div>
          </div>
        </Card>
